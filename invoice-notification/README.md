...
| `triggers.js` | トリガー設定（初回のみ手動実行） |
| `appsscript.json` | GAS マニフェスト（タイムゾーン: Asia/Tokyo） |

## 前提条件

[ルートの README](../README.md#開発環境) に従って、以下を済ませてください。
...
`clasp run` でトリガー設定を行う場合は、push 後に実行可能 API のデプロイが必要です（[手順](../README.md#実行可能-api-としてデプロイ)）。

### 8. トリガーを設定

以下のいずれかの方法で、毎日午前8時に `sendInvoiceNotifications` が自動実行されるトリガーを設定します。
