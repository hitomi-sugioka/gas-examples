...
GAS トリガーの設定・一覧・削除を一元管理するテンプレートです。
...
以下のいずれかの方法で、`TRIGGER_CONFIGS`（`config.js`）に定義されたトリガーを一括設定します。デフォルトでは時間主導型5種 + イベント型2種 = 計7件のダミー関数を設定します。

#### 方法 A: スプレッドシートのカスタムメニューから実行

スプレッドシートを開き、「**トリガー管理**」メニュー →「**トリガーを設定**」を選択します。確認ダイアログで「はい」を選ぶとトリガーが一括作成されます。
...
`config.js` の `TRIGGER_CONFIGS` を編集して、トリガーの追加・変更・削除ができます。
...
- [GASのトリガーをコードで一元管理する方法【テンプレート付き】](https://www.sugiokasystem.co.jp/blog/gas-trigger-code-management)
