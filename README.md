...
Google Apps Script（GAS）の実務向けサンプルコード集です。

[杉岡システム株式会社](https://www.sugiokasystem.co.jp) の公式ブログ記事に関連するサンプルコードを公開しています。  
スプレッドシート連携・トリガー管理・メール通知など、業務自動化で利用しやすい題材を扱っています。
...
| [trigger-manager](./trigger-manager/) | トリガーの設定・一覧・削除をコードで一元管理 |
...
事前に、対象スプレッドシートを開いて「拡張機能」→「Apps Script」で GAS プロジェクトを作成し、エディタの **設定**（歯車アイコン）から **スクリプト ID** をコピーしておきます。

```bash
cd invoice-notification          # または trigger-manager
cp .clasp.json.example .clasp.json   # コピーした scriptId に書き換え
clasp push
```

初回 push 後は GAS エディタから対象関数を手動実行して権限承認を済ませてください。  
サンプル固有のセットアップ（CSV インポート・`config.js` 編集・トリガー設定など）は、各サンプルの README を参照してください。
...
#### Deploying as Executable API

`clasp run` を使用するには、事前に GAS エディタで実行可能 API としてデプロイしておく必要があります。  
なお、実行されるコードはデプロイされたバージョンではなく、`clasp push` によって反映された最新のスクリプト（HEAD）です。
...
#### Creating OAuth Client ID (for clasp run)
...
本リポジトリは技術検証・参考用途として公開しています。  
Issue / Pull Request / 個別サポートには対応していません。  
必要に応じて内容を調整のうえご利用ください。
...
- Hitomi Sugioka / [杉岡システム株式会社](https://www.sugiokasystem.co.jp)  
- 公式ブログ: [https://www.sugiokasystem.co.jp/blog](https://www.sugiokasystem.co.jp/blog)

※本リポジトリは執筆担当者の GitHub アカウントで公開しています。
