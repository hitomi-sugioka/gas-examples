...
`clasp run` を使うには、**`clasp push` でコードを反映した後に**、GAS エディタから実行可能 API としてデプロイが必要です。push 前にデプロイしても、コードが反映されません。

1. `clasp push` でコードを GAS プロジェクトに反映
2. GAS エディタで **デプロイ** → **新しいデプロイ** を開く
3. 種類の選択で **実行可能 API** を選択
4. 「アクセスできるユーザー」を設定 — **「自分のみ」では `clasp run` が動作しません**。Google Workspace の場合は「組織内の全員」、個人アカウントの場合は「全員」を選択してください
